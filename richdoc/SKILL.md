...
For handbooks, runbook sets, or reference manuals that don't fit in one file: put an `<rd-toc>` with `<rd-chapter>` children in every page. The same block lives in every file; `<rd-toc>` handles active-chapter detection, prev/next nav, and the sidebar at runtime. No build step, no cross-file fetch. See [references/multi-file-books.md](references/multi-file-books.md) and `examples/book/`.
...
- **Books duplicate the chapter list** by design — re-ordering means editing every chapter file. `richdoc lint` catches stale `href`s.
