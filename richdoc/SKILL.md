...
| --- | --- | --- |
| `<rd-page>` | `theme?` (`editorial-warm\|graphite-modern`), `mode?` (`light\|dark\|auto`), `width?` (`narrow\|standard\|wide\|full`, default `standard`), `prefs?` (`off`) | Outer container. Exactly one per doc, directly under `<body>`. A floating **preview picker** auto-appears in the bottom-right corner so readers can switch theme / mode / width at runtime; selections persist in `localStorage` per origin+path. Set `prefs="off"` to suppress it. **Themes carry their own type stacks**: `editorial-warm` uses Fraunces (display) + Geist (body) + Fira Code (mono); `graphite-modern` uses Space Grotesk + Inter + JetBrains Mono — a more modernist/technical voice. Responsiveness is container-driven — multi-column layouts, stat cards, and the TOC adapt to the chosen content width, not the viewport. |
