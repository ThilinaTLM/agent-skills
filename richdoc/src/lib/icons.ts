/**
 * Vendored Lucide icon set (MIT licensed). Hand-curated subset that
 * covers the cases agents actually need in editorial documents:
 * status markers, navigation arrows, document/file glyphs, common
 * UI decorations. Each value is the inner SVG markup for a 24x24
 * viewBox with stroke="currentColor"; the wrapping <svg> element is
 * supplied by <rd-icon>.
 *
 * Adding an icon: paste its inner markup from https://lucide.dev,
 * keep stroke attributes off (they're set on the outer <svg>), and
 * the schema enum updates automatically via ICON_NAMES.
 */
