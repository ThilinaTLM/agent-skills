# Lucide icons available to `<rd-icon name="…">`

Auto-generated from `lucide-static@1.16.0` — do not edit. Re-run
`pnpm build` from `richdoc/richdoc-lib/` after bumping the version pin in
`src/components/icon/lucide-version.ts`.

Inline a glyph anywhere in a richdoc with:

```html
<rd-icon name="alert-triangle"></rd-icon>
```

Optional attributes: `size` (`sm` / `md` / `lg`, default `md`), `label`
(sets `aria-label` + `<title>` for meaningful icons).
...
is marked `data-rd-icon-missing`.
