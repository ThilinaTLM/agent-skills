...
2. **`src/schema-registry.ts`** — add a schema import and a registry entry. The order in `SCHEMA_ENTRIES` determines the order in `assets/schema.json`, `richdoc components`, and the SKILL.md table — pick a position that fits the existing grouping (Structure, Information blocks, Comparison & code, Sequenced & interactive).

3. **`src/registry.ts`** — add the implementation import and append `<name>.register` to `REGISTRATIONS`.

That's it. Run `bun run build` from the `richdoc/` root. The build:
...
| CLI linter — reads schema | `richdoc-cli/src/richdoc_cli/commands/lint.py` |
| CLI introspection | `richdoc-cli/src/richdoc_cli/commands/components.py` |
| CLI launcher (bash / PowerShell) | `richdoc-cli/richdoc`, `richdoc-cli/richdoc.ps1` |
| CLI project config | `richdoc-cli/pyproject.toml` (uv-managed Python) |
