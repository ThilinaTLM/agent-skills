"""Dispatch table for the markdown converter.

Importing this module populates `converter.HANDLERS` with every plain-HTML
and rd-* handler. The converter consults that dict at runtime via
`_Converter.render()`. Splitting the table from the handler code lets us
re-organise either side without churning the dispatch.
"""
...
HANDLERS.update({
    ...
    "rd-footnotes": p._h_drop,  # auto-generated by JS; we manage our own.
